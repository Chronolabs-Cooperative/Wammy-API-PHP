 ...
 */
	...
	$mode = isset($_GET['mode'])?(string)$_GET['mode']:'ham';
	...
	if (count($errors)>0) {
		...
		foreach($resource as $key => $value)
			$template = str_replace("%key", $value, $template);
		$fname = constant("DIR_TRAINING_".strtoupper($mode)) . DIRECTORY_SEPARATOR . $resource['testid'] . '@' . $resource['apidomain'] . '.msg';
		mkdirSecure(constant("DIR_TRAINING_".strtoupper($mode)));
		writeRawFile($fname, $template);
		...
		if (isset($_REQUEST['callback']) && !empty($_REQUEST['callback']))
		{
			...
			writeRawFile($fname, json_encode(array('request'=>$_REQUEST, 'data'=>$data, 'resource' => $resource, 'peerid'=>$GLOBALS['peerid'])));
		}
		...
		{
			if (!empty($peer['api-uri-'.$mode]))
				setCallBackURI($peer['api-uri-'.$mode], 290, 290, array_merge($_REQUEST, array('testid'=>$resource['testid'], 'peerid'=>$GLOBALS['peerid'])));
		}
