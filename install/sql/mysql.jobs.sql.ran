...
CREATE TABLE `jobs` (
  ...
  `state` enum('realtime','queued') NOT NULL DEFAULT 'realtime',
  `mode` enum('textual','image','unknown') NOT NULL DEFAULT 'unknown',
  `typal` enum('training','testing','unknown') NOT NULL DEFAULT 'unknown',
  `balance` enum('spam','ham','crass') NOT NULL DEFAULT 'ham',
  `hashing` varchar(32) NOT NULL,
  `sentences` mediumint(32) NOT NULL DEFAULT '0',
  `headers` mediumint(32) NOT NULL DEFAULT '0',
  `questions` mediumint(32) NOT NULL DEFAULT '0',
  `exclamations` mediumint(32) NOT NULL DEFAULT '0',
...
-- Dump completed on 2017-10-08  2:18:21
